...
type Note = { id: string; notebookId: string | null; title: string; content: string; tags: string; updatedAt: string };
type Meeting = {
  id: string;
...
type Task = { id: string; title: string; sourceType: string | null; sourceId: string | null; completed: boolean; dueDate: string | null; createdAt: string };
...
function newTask(title: string, sourceType: string | null = null, sourceId: string | null = null): Task {
  return { id: crypto.randomUUID(), title, sourceType, sourceId, completed: false, dueDate: null, createdAt: new Date().toISOString() };
}
...
export function App() {
  ...
  const filteredMeetings = useMemo(
    () => workspace.meetings.filter((meeting) => `${meeting.title} ${meeting.transcript} ${meeting.minutes}`.toLowerCase().includes(query.toLowerCase())),
  ...
  const addTask = async (sourceType: string | null = null, sourceId: string | null = null) => {
    ...
    await invoke("upsert_task", { task: newTask(title.trim(), sourceType, sourceId) });
    await reload();
    notify("已加入待办");
  };
      ...
      mediaRecorder.onstop = () => {
        void (async () => {
          try {
            ...
            await invoke("save_recording", { meetingId, dataUrl, durationSeconds: recordingSecondsRef.current });
            const latest = await reload();
            setSelectedMeeting(latest.meetings.find((meeting) => meeting.id === meetingId) ?? null);
  ...
  const transcribeMeeting = async () => {
    ...
    try {
      ...
      const meeting = await invoke<Meeting>("transcribe_meeting", { meetingId: selectedMeeting.id });
      setSelectedMeeting(meeting);
      await reload();
  ...
  const analyzeMeeting = async () => {
    ...
    try {
      ...
      const result = await invoke<AnalysisResult>("analyze_meeting", { meetingId: selectedMeeting.id });
  ...
  const saveAiSettings = async () => {
    try {
      ...
      setAiSettings(saved);
      setApiKey("");
      notify(saved.isConfigured ? "智能纪要服务已保存，密钥已存入 Windows 凭据库" : "服务地址已保存；填写 API 密钥后才会启用智能纪要");
    } catch (error) {
  ...
  if (loading) return <div className="loading"><LoaderCircle size={26} className="spin" />正在打开知记…</div>;
    ...
    <div className="app-shell">
      ...
      <aside className="sidebar">
        ...
        <nav className="nav-list">
          <NavItem active={view === "home"} icon={<House size={18} />} onClick={() => setView("home")}>首页</NavItem>
          <NavItem active={view === "meetings"} icon={<UsersRound size={18} />} onClick={() => setView("meetings")}>会议</NavItem>
          <NavItem active={view === "notes"} icon={<NotebookPen size={18} />} onClick={() => setView("notes")}>笔记本</NavItem>
          <NavItem active={view === "tasks"} icon={<CheckCircle2 size={18} />} onClick={() => setView("tasks")}>待办</NavItem>
        </nav>
        ...
        <button className="settings-link" onClick={() => setView("settings")}><Settings size={18} />设置与智能功能</button>
      </aside>
      <main className="main-content">
        <header className="page-header">
          ...
          <label className="search-box"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索会议和笔记" /></label>
        </header>
...
function TitleBar() {
  ...
  return <div className="title-bar" data-tauri-drag-region><span data-tauri-drag-region>知记 · 本地资料库</span><div className="window-controls" data-tauri-drag-region="false"><button onClick={() => void appWindow.minimize()} title="最小化"><Minimize2 size={15} /></button><button onClick={() => void appWindow.toggleMaximize()} title="最大化"><Maximize2 size={14} /></button><button className="close-window" onClick={() => void appWindow.close()} title="关闭"><X size={16} /></button></div></div>;
}

function NavItem({ active, icon, onClick, children }: { active: boolean; icon: ReactNode; onClick: () => void; children: string }) {
...
function Home({ workspace, onMeeting, onNote, onOpenMeeting, onOpenNote }: { workspace: Workspace; onMeeting: () => void; onNote: () => void; onOpenMeeting: (meeting: Meeting) => void; onOpenNote: (note: Note) => void }) {
  ...
  return <div className="page-grid home-grid"><section className="welcome-card"><div><span className="eyebrow">录音 · 本地转写 · 纪要 · 行动</span><h2>把一次会议，变成下一步行动。</h2><p>会议录音、转写原文和笔记默认留在你的电脑。需要智能纪要时，再由你主动选择已配置的服务处理文字稿。</p></div><div className="welcome-actions"><button className="primary-button" onClick={onMeeting}><Mic size={17} />开始会议</button><button className="secondary-button" onClick={onNote}><FileText size={17} />新建笔记</button></div></section><section className="stats-row"><Stat icon={<UsersRound />} label="全部会议" value={workspace.meetings.length} /><Stat icon={<NotebookPen />} label="全部笔记" value={workspace.notes.length} /><Stat icon={<CheckCircle2 />} label="待完成" value={openTasks.length} /></section><section className="panel recent-panel"><div className="panel-title"><h3>最近会议</h3><button onClick={onMeeting}>新建 <Plus size={14} /></button></div>{workspace.meetings.slice(0, 4).map((meeting) => <button className="recent-row" key={meeting.id} onClick={() => onOpenMeeting(meeting)}><span className="round-icon purple"><UsersRound size={16} /></span><span><strong>{meeting.title}</strong><small>{dateTime(meeting.startedAt)} · {meeting.status}</small></span><ChevronRight size={17} /></button>)}{workspace.meetings.length === 0 && <Empty label="还没有会议，开始记录第一场吧。" />}</section><section className="panel recent-panel"><div className="panel-title"><h3>最近笔记</h3><button onClick={onNote}>新建 <Plus size={14} /></button></div>{workspace.notes.slice(0, 4).map((note) => <button className="recent-row" key={note.id} onClick={() => onOpenNote(note)}><span className="round-icon yellow"><FileText size={16} /></span><span><strong>{note.title}</strong><small>{note.content || "空白笔记"}</small></span><ChevronRight size={17} /></button>)}{workspace.notes.length === 0 && <Empty label="还没有笔记。" />}</section></div>;
}
...
function Notes({ notes, notebooks, note, onSelect, onCreate, onChange, onSave }: { notes: Note[]; notebooks: Notebook[]; note: Note | null; onSelect: (note: Note) => void; onCreate: () => void; onChange: (note: Note) => void; onSave: () => void }) {
  return <div className="split-layout"><section className="list-pane"><div className="pane-heading"><div><h2>全部笔记</h2><small>{notes.length} 条笔记</small></div><button className="round-add" onClick={onCreate}><Plus size={18} /></button></div>{notes.map((item) => <button className={`note-item ${note?.id === item.id ? "selected" : ""}`} onClick={() => onSelect(item)} key={item.id}><FileText size={17} /><span><strong>{item.title}</strong><small>{item.content || "空白笔记"}</small></span></button>)}{notes.length === 0 && <Empty label="还没有笔记。" />}</section><section className="editor-pane note-editor">{note ? <><div className="editor-top"><div><input className="title-input" value={note.title} onChange={(event) => onChange({ ...note, title: event.target.value })} /><small>上次编辑于 {dateTime(note.updatedAt)}</small></div><button className="primary-button" onClick={onSave}><Check size={15} />保存</button></div><div className="note-meta"><FolderOpen size={15} /><select value={note.notebookId ?? ""} onChange={(event) => onChange({ ...note, notebookId: event.target.value || null })}><option value="">未分类</option>{notebooks.map((notebook) => <option value={notebook.id} key={notebook.id}>{notebook.name}</option>)}</select><input value={note.tags} onChange={(event) => onChange({ ...note, tags: event.target.value })} placeholder="标签，用逗号分隔" /></div><textarea className="note-content" value={note.content} onChange={(event) => onChange({ ...note, content: event.target.value })} placeholder="开始书写…" /></> : <Empty label="选择一条笔记，或新建笔记。" />}</section></div>;
}

function EditorField({ label, hint, value, onChange, placeholder }: { label: string; hint: string; value: string; onChange: (value: string) => void; placeholder: string }) {
  return <div className="editor-field"><div><h3>{label}</h3><small>{hint}</small></div><textarea value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} /></div>;
}

function Tasks({ tasks, onAdd, onToggle }: { tasks: Task[]; onAdd: () => void; onToggle: (task: Task) => void }) {
  const open = tasks.filter((task) => !task.completed);
  const done = tasks.filter((task) => task.completed);
  return <div className="tasks-page"><div className="tasks-intro"><div><h2>专注下一步</h2><p>智能纪要提取的行动项，会自动出现在这里。</p></div><button className="primary-button" onClick={onAdd}><Plus size={16} />新建待办</button></div><TaskGroup title="待完成" tasks={open} onToggle={onToggle} /><TaskGroup title="已完成" tasks={done} onToggle={onToggle} /></div>;
}

function TaskGroup({ title, tasks, onToggle }: { title: string; tasks: Task[]; onToggle: (task: Task) => void }) {
  return <section className="task-group"><div className="group-heading"><h3>{title}</h3><span>{tasks.length}</span></div>{tasks.length ? tasks.map((task) => <label className={`task-row ${task.completed ? "done" : ""}`} key={task.id}><input type="checkbox" checked={task.completed} onChange={() => onToggle(task)} /><span className="checkmark">{task.completed && <Check size={13} />}</span><span>{task.title}</span>{task.sourceType && <small>{task.sourceType === "meeting" ? "会议" : "笔记"}</small>}</label>) : <Empty label={title === "待完成" ? "没有待办，给自己留一点空白。" : "完成的事项会保留在这里。"} />}</section>;
}

function SettingsView({ workspace, aiSettings, asrStatus, apiKey, processing, onAiChange, onApiKeyChange, onSaveAi, onClearAiKey, onDownloadAsr, onCreateNotebook, onBackup }: { workspace: Workspace; aiSettings: AiSettings; asrStatus: LocalAsrStatus; apiKey: string; processing: Processing; onAiChange: (settings: AiSettings) => void; onApiKeyChange: (key: string) => void; onSaveAi: () => void; onClearAiKey: () => void; onDownloadAsr: () => void; onCreateNotebook: () => void; onBackup: () => void }) {
  return <div className="settings-page"><section className="settings-hero"><span className="round-icon purple"><Sparkles size={19} /></span><div><h2>语音识别在本机，智能纪要由你决定</h2><p>录音与语音转写始终在这台电脑完成。只有点击“生成智能纪要”时，才会把转写文字发送给你配置的服务；绝不会上传录音。</p></div></section><section className="settings-card ai-settings"><div><h3>本地中文语音模型</h3><p>SenseVoiceSmall Q8 + FSMN-VAD。模型仅首次下载，之后离线运行；适合普通 Windows 电脑的中文会议转写。</p></div><span className={`ai-status ${asrStatus.installed ? "ready" : ""}`}>{asrStatus.installed ? `已安装 ${asrStatus.modelSizeMb} MB` : "尚未下载"}</span><div className="settings-actions"><button className="primary-button" disabled={processing !== null || asrStatus.installed} onClick={onDownloadAsr}>{processing === "downloading" ? <LoaderCircle className="spin" size={16} /> : <Mic size={16} />}{processing === "downloading" ? "正在下载模型" : asrStatus.installed ? "模型已就绪" : "下载本地模型"}</button>{!asrStatus.runtimeAvailable && <small className="runtime-warning">语音运行时将在 Windows 安装包中提供。</small>}</div></section><section className="settings-card ai-settings"><div><h3>智能纪要服务（可选）</h3><p>兼容 OpenAI 的聊天补全接口。只在你点击分析后发送转写文本，不发送录音；API 密钥保存到 Windows 凭据库，不写入 SQLite。</p></div><span className={`ai-status ${aiSettings.isConfigured ? "ready" : ""}`}>{aiSettings.isConfigured ? "已配置" : "未配置"}</span><div className="ai-grid"><label>服务地址<input value={aiSettings.baseUrl} onChange={(event) => onAiChange({ ...aiSettings, baseUrl: event.target.value })} placeholder="https://api.openai.com/v1" /></label><label>纪要模型<input value={aiSettings.analysisModel} onChange={(event) => onAiChange({ ...aiSettings, analysisModel: event.target.value })} placeholder="gpt-4o-mini" /></label><label>API 密钥<input type="password" value={apiKey} onChange={(event) => onApiKeyChange(event.target.value)} placeholder={aiSettings.isConfigured ? "已保存；留空即可保留原密钥" : "粘贴 API 密钥"} /></label></div><div className="settings-actions"><button className="primary-button" onClick={onSaveAi}><Check size={16} />保存智能纪要设置</button>{aiSettings.isConfigured && <button className="secondary-button" onClick={onClearAiKey}>删除密钥</button>}</div></section><section className="settings-card"><div><h3>笔记本</h3><p>用笔记本把会议和普通笔记归类。</p></div><button className="secondary-button" onClick={onCreateNotebook}><Plus size={16} />新建笔记本</button></section><section className="settings-card"><div><h3>本地资料库</h3><p>当前保存 {workspace.meetings.length} 场会议、{workspace.notes.length} 条笔记和 {workspace.tasks.length} 项待办。数据使用 SQLite 存储在 Windows 应用资料目录。</p></div></section><section className="settings-card"><div><h3>立即备份</h3><p>创建一个包含 SQLite 资料库与全部会议录音的本机副本。</p></div><button className="secondary-button" onClick={onBackup}><Archive size={16} />创建备份</button></section></div>;
}

function Empty({ label }: { label: string }) {
  return <div className="empty-state"><MoreHorizontal size={22} /><p>{label}</p></div>;
}
